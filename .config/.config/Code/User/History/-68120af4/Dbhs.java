...
public class ChessboardGUI extends JFrame
{
  ...
  public void init_main_window()
  {
    ...
    // Use GridBagLayout and constraints
    setLayout(new GridBagLayout());

    int row = 0;
    addWithConstraints(topBar, row++, topBar.BAR_HEIGHT);
    addWithConstraints(Box.createRigidArea(TOP_SPACER_SIZE), row++, TOP_SPACER_SIZE.height);
    addWithConstraints(spacedBoard, row++, board.BOARD_SIZE.height);
    addWithConstraints(Box.createRigidArea(BOTTOM_SPACER_SIZE), row++, BOTTOM_SPACER_SIZE.height);
    addWithConstraints(bottomBar, row, bottomBar.BAR_HEIGHT);
  }

  private void addWithConstraints(java.awt.Component comp, int gridy, int height) {
